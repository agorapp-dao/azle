...
async function compileTypeScriptToJavaScript(main, canisterConfig) {
    try {
...
async function bundleAndTranspileJs(ts) {
    const jsBundled = await bundleFromString(ts);
    const jsTranspiled = await transpile(jsBundled);
...
async function bundleFromString(ts) {
